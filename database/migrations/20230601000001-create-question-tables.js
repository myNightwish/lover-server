...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    await queryInterface.createTable('questions', {
      ...
      options: { type: Sequelize.JSON, comment: '选项，用于options类型' },
    ...
    // 创建用户话题进度表
    await queryInterface.createTable('user_topic_progresses', {
      id: { type: Sequelize.INTEGER, primaryKey: true, autoIncrement: true },
    ...
    // 创建用户问题回答表
    await queryInterface.createTable('user_question_answers', {
      id: { type: Sequelize.INTEGER, primaryKey: true, autoIncrement: true },
      user_id: { type: Sequelize.INTEGER, comment: '用户ID' },
      ...
      answer_value: { type: Sequelize.JSON, comment: '回答内容' },
      created_at: Sequelize.DATE,
      updated_at: Sequelize.DATE
    });

    // 创建版本迁移日志表
    await queryInterface.createTable('version_migration_logs', {
      id: { type: Sequelize.INTEGER, primaryKey: true, autoIncrement: true },
      user_id: { type: Sequelize.INTEGER, comment: '用户ID' },
      topic_id: { type: Sequelize.INTEGER, comment: '话题ID' },
  ...
  down: async (queryInterface) => {
    await queryInterface.dropTable('version_migration_logs');
    await queryInterface.dropTable('user_question_answers');
    await queryInterface.dropTable('user_topic_progresses');
